...
async function createCareDataMarkers() {
  for (var i = 0; i < Care_positions.length; i++) {
    var imageSize = new kakao.maps.Size(122, 122),
      imageOptions = {
        spriteOrigin: new kakao.maps.Point(10, 0),
        spriteSize: new kakao.maps.Size(122, 122),
      };
...
function buttonClicked(buttonId) {
  console.log("버튼 " + buttonId + "이(가) 클릭되었습니다.");
  if (buttonId == 1) {
    for (var i = 0; i < MedicalMarkers.length; i++) {
      if (Medical_TH[i] == "치과") {
        MedicalMarkers[i].setMap(map);
      } else {
        MedicalMarkers[i].setMap(null);
      }
    }
  } else if (buttonId == 2) {
    for (var i = 0; i < MedicalMarkers.length; i++) {
...
function checkboxChanged(checkboxId) {
  ...
  if (checkbox.checked) {
    console.log("체크박스 " + checkboxId + "이(가) 선택되었습니다.");
  } else {
    console.log("체크박스 " + checkboxId + "이(가) 선택 해제되었습니다.");
  }
}
